...
##########################################################################################
######################### IMP Modeling Script for MHM Complex ###########################
##########################################################################################
...
po.system.title = "Integrative structure of the human MHM complex"
...
for prot in ["MBD3"]:
    for cp in [0,1]:
        set2 += IMP.atom.Selection(root_hier, molecule=prot, copy_index=cp, residue_indexes=range(1,296)).get_selected_particles()
for prot in ["P66A"]:
    for cp in [0,1]:
        set2 += IMP.atom.Selection(root_hier, molecule=prot, copy_index=cp, residue_indexes=range(136,179)).get_selected_particles()
# print(set2)
...
s = po.system
import ihm.dumper
with open('model_init_mhm.cif', 'w') as fh:
...
last_step.num_models_end = 600_000 #20,000 models per run and 30 independent runs (8 cores per run)
...
analysis.steps.append(ihm.analysis.ClusterStep(
                      ...
                      num_models_end=28451))        # nModels in cluster0
...
m = IMP.Model()
inf1 = RMF.open_rmf_file_read_only('../../results/mhm/cluster.0/cluster_center_model.rmf3')
...
loc_density_list={"MTA1.0":["LPD_MTA1.0_BAH_1-164",
                            "LPD_MTA1.0_ELM2_SANT_165-333",
                            "LPD_MTA1.0_mid_334-431"],
                  "MTA1.1":["LPD_MTA1.1_BAH_1-164",
                            "LPD_MTA1.1_ELM2_SANT_165-333",
                            "LPD_MTA1.1_mid_334-431"],
                  "HDAC1.0":["LPD_HDAC1.0_1-376",
                            "LPD_HDAC1.0_C-term_377-482"],
                  "HDAC1.1":["LPD_HDAC1.1_1-376",
                            "LPD_HDAC1.1_C-term_377-482"],
                  "MBD3.0":["LPD_MBD3.0_N-term",
                            "LPD_MBD3.0_mid_uns",
                            "LPD_MBD3.0_mid_struc",
                            "LPD_MBD3.0_C-term"],
                  "MBD3.1":["LPD_MBD3.1_N-term",
                            "LPD_MBD3.1_mid_uns",
                            "LPD_MBD3.1_mid_struc",
                            "LPD_MBD3.1_C-term"],
                  "P66A.0":["LPD_P66A.0"],
                  "P66A.1":["LPD_P66A.1"]}
...
for prot in loc_density_list:
      ...
      for domain_density in loc_density_list[prot]:
          loc = ihm.location.OutputFileLocation('../../results/mhm/cluster.0/'+domain_density+'.mrc')
...
with open('model_mhm.cif', 'w') as fh:
    ihm.dumper.write(fh, [po.system])

import ihm.reader
with open('model_mhm.cif') as fh:
